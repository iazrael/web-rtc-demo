...
let published = false;
let isLogin = false;
const useLocalStreamList: StreamInfo[] = [];
let publishStreamId = '';
...
$(async () => {
    ...
    $('#publish').click(async () => {
        ...
        const result = zg!.startPublishingStream($('#streamId').val() as string, previewStream ? previewStream : previewVideo.srcObject as MediaStream);
        published = true;
        publishStreamId = $('#streamId').val() as string;
    ...
    $('#leaveRoom').click(async () => {
        ...
        if (published) {
            zg!.stopPublishingStream($('#streamId').val() as string);
            published = false;
        }
...
let lastInfo: LastInfo = {};

function resetInstance(appId: number) {
        ...
        zg.on('roomStreamUpdate', async (roomID: string, updateType: 'ADD' | 'DELETE', streamList: StreamInfo[], extendedData: any) => {
            ...
            if (updateType == 'ADD') {
                for (let i = 0; i < streamList.length; i++) {
                    ...
                    const handlePlaySuccess = (streamItem: StreamInfo) => {
                        let video: HTMLVideoElement | HTMLAudioElement;
                        const bro = getBrowser();
                        if (bro == 'Safari' && (playOption as any).video === false) {
                    ...
                    const playOption: any = {};
                    const _selectMode = $('#playMode option:selected').val() as string;
...
async function login(roomId: string): Promise<void> {
    ...
    const loginOptions: RoomLoginOptions = { 
        userUpdate: true 
    };
